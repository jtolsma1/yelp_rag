...
class SummarizeRelevantReviewText:
    ...
    def retrieve_relevant_text_summaries_from_ollama(self,relevant_chunk_df,restaurant_ids,status_cb:Optional[StatusCB] = None):
        """
        ...
        emit({"type":"start","total":total,"message":f"Summarizing reviews for {total} restaurants."})
        ...
        for restaurant_no,id in enumerate(restaurant_ids):
            ...
            emit({
                    "type":"restaurant_done",
                    "restaurant_no":restaurant_no+1,"total":total,
                    "name":restaurant_name,
                    "message":f"{restaurant_no+1}/{total} Done: {restaurant_name}"
                    })
        ...
        emit({"type":"done","total":total,"message":"Restaurant summarization complete"})

        return summary_df

    def summarize_relevant_review_text(self,status_cb: Optional[StatusCB] = None):
        """
        ...
        summary_df = self.retrieve_relevant_text_summaries_from_ollama(relevant_chunk_df,restaurant_ids,status_cb)
